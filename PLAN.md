...
| D41 | 2026-07-23 | 在首次操作說明、CMS 追問與 unknown CMS 報告三個節點，以「照管中心評估後核定的長照需要等級（第 2–8 級）」白話解釋 CMS，並明示尚未評估可回答「不知道」 | 一般使用者未必認識 CMS；在第一次接觸與實際需要回答的位置提供短句，可減少疑惑，又不增加新的大型說明區塊或打斷主要操作流程 |
| D42 | 2026-07-23 | 每輪追問下方固定顯示獨立的「下一步：在這裡回答」操作區，明示可分多次補充，送出按鈕標示「繼續下一題」，並在每次追問更新後自動聚焦回答欄 | 首次使用者容易把追問卡視為靜態結果，不知道仍可多輪輸入；把下一個動作、後續結果與可輸入位置放在同一視覺區塊，可降低尋找成本並建立清楚的多輪節奏 |
...
- `ConsistencyStatus`：只允許 `CONSISTENT`、`DRIFT_DETECTED`；比較 manifest、runtime metadata／業務常數、README、離線 fixtures 與測試斷言。
- `ConsistencyCheckResult`：保存確定性跨檔檢查的問題清單與 `writes_performed=false`。
- `ReviewDecision`：只允許 `PENDING`、`APPROVED`、`REJECTED`；產生或決定複核結果都不得修改現行規則。
...
- [x] `rule-sources-v1.json` 以 manifest `2026-07-23.1` 鎖定四個官方來源、exact URL 白名單、extractor 版本、raw hash 與必要語意快照。
- [x] 離線 fixture 與 14 項 checker 測試可穩定重現 `VERIFIED_SNAPSHOT`、`REVIEW_REQUIRED`、`CHECK_UNAVAILABLE`。
- [x] 第一次線上 smoke 的四個來源均為 HTTP 200、`VERIFIED_SNAPSHOT`、`changed_fields=[]`、`writes_performed=false`；證據寫入忽略版控的 `artifacts/rule-audit/2026-07-23-online.json`。
- [x] 測試證明 checker 不寫入 `rules.py`、`eligibility.py` 或 `copay.py`，且 audit 套件未進入 Gradio／Agent runtime。
- [x] `uv lock --check`、`uv pip check`、完整 pytest **528 passed** 與 sdist／wheel build 均成功；wheel 包含 manifest 資料檔。
- [x] 作者逐項檢閱四個官方來源、狀態與實跑證據，並明確回覆「v0.2-P1 驗收通過」後核准進入 v0.2-P2。
...
- [x] P1 JSON 證據可離線產生正體中文 Markdown 複核報告，逐來源列出狀態、舊值、新值、影響規則與建議測試。
- [x] manifest、runtime metadata／業務常數、README、離線 fixtures 與測試斷言具備確定性一致性檢查。
- [x] 報告不依賴 LLM、不呼叫網路或付費 API，並明示 `writes_performed=false`。
- [x] `REVIEW_REQUIRED`、一致性漂移或不安全證據會以非零 exit code 阻擋通過。
- [x] approve／reject 皆不修改規則；測試證明人工拒絕後現行快照不變。
- [x] 作者檢閱 P2 複核報告與實跑證據，明確回覆「v0.2-P2 驗收通過」後核准進入 v0.2-P3。
...
- [x] Windows CI、手動來源稽核、本機發布門檻、公開連結、唯讀 Space 桌面／390 px 手機版檢查通過。
