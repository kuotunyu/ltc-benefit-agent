...
![tests 485 passed](https://img.shields.io/badge/tests-485%20passed-185A45?style=flat-square)
...
workflow middleware 現在只在已有成功、可稽核的工具證據後接續固定流程：複製已驗證的資格／金額參數建立草稿，再把 registry 回傳的 `report_id` 與 Markdown 原文送入既有 HITL。它不解析使用者敘述、不補造欄位，也不計算資格或金額。F1 的 Ollama Hermes template 另依原模型官方 chat template 補上 tool response 的工具名。

| 指標 | 3B 初始 | 3B 強化後 | 12B 初始 | 12B 強化後 |
|---|---:|---:|---:|---:|
| 缺漏追問正確 | 18 / 20 | 16 / 20 | 12 / 20 | 17 / 20 |
| 工具選擇正確 | 0 / 20 | 12 / 20 | 5 / 20 | 17 / 20 |
| 工具參數正確 | 12 / 20 | 16 / 20 | 17 / 20 | 16 / 20 |
| 金額條件無誤（20 題口徑） | 14 / 20 | 19 / 20 | 19 / 20 | 18 / 20 |
| PII 洩漏次數 | 0 | 0 | 0 | 0 |
| HITL 正確觸發 | 0 / 20 | 12 / 20 | 4 / 20 | 17 / 20 |
| 端到端通過 | 0 / 20 | 10 / 20 | 3 / 20 | 12 / 20 |

真正需要試算的 13 題中，強化後 F1 為 `12 / 13`、12B adapter 為 `11 / 13` 金額完全一致。雲端模式尚未在這版 workflow 上重跑，因此上方初始表的雲端 `7 / 20` 只能當歷史基線，不能直接和地端強化後數字宣稱同版優劣。重跑雲端固定集前仍須重新估算並核准費用。
...
- 確定性接續大幅改善兩個地端模式的建稿與 HITL：F1 端到端由 `0 / 20` 到 `10 / 20`，12B adapter 由 `3 / 20` 到 `12 / 20`。
- F1 的剩餘失敗集中在前段資格邊界與資料蒐集；追問反而由 `18 / 20` 降為 `16 / 20`，所以仍不能當成可無人監督的正式服務。
- 12B adapter 的工具選擇與 HITL 分別提升到 `17 / 20`，但工具參數與金額各有一點退步；其中不合法的報告改寫仍被 registry 拒絕並誠實計為失敗。
- 初始雲端模式為 `7 / 20`；新 workflow 尚未重跑雲端固定集，不能用舊雲端數字宣稱目前三者排名。
- 所有已跑模式的 PII 洩漏仍為 0；安全閘門與人工核准沒有為了提高通過率而放寬。

這張表衡量的是「LLM 能否可靠地主持多輪工作流」，不是 Python 資格與金額規則本身的單元測試成績。後者另由下方 490 項 pytest 與 336 組規則／金額主矩陣驗證。
...
| 真實本機 12B 固定 20 題（強化後） | 12 / 20 端到端；需試算題 11 / 13 金額一致；0 次 PII 洩漏 |
| 真實本機 3B 固定 20 題（強化後） | 10 / 20 端到端；需試算題 12 / 13 金額一致；0 次 PII 洩漏 |
